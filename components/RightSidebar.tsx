...
const RightSidebar = () => {
  ...
  return (
      <section className={cn("right_sidebar text-white-1", {
        ...
        <div className='flex flex-col gap-6'>
          {topPodcasters?.slice(0, 5).map((podcaster) => (
            ...
            <div className="flex items-center">
              <p className="text-12 font-normal text-white-1">{podcaster.totalPodcasts} podcast{podcaster.totalPodcasts > 1 ? 's' : ''}</p>
            </div> 
          </div>
          ))}
        </div>
